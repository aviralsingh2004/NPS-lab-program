...
socket.on("transfer-request", (data) => {
  ...
  if (
    confirm(`User ${sourceId} wants to send you file: ${fileName}. Accept?`)
  ) {
...
socket.on("receive-data", (data) => {
  ...
  if (key) {
    try {
      ...
      // Decrypt the data
      const decryptedBase64 = CryptoJS.AES.decrypt(encryptedData, key).toString(
        CryptoJS.enc.Utf8
      );
      ...
      addTransferStatus(
        `File "${downloadFileName}" received and decrypted successfully. Check your Downloads folder.`
      );
